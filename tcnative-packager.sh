...
V=1.1.33                # this is the vesion of tcnative to build/package
RELEASE=0               # An internal package build number
NAME="tcnative"         # The resulting package name
...
if [ ! -f tomcat-native-$V-src.tar.gz ];then
  ...
  wget -q "http://www.motorlogy.com/apache/tomcat/tomcat-connectors/native/$V/source/tomcat-native-$V-src.tar.gz"
fi
...
cd tomcat-native-$V-src/jni/native
